...
func initialize():
	...
	var traversable_tiles = get_used_cells()
...
func _add_traversable_tiles(traversable_tiles):
	...
	for tile in traversable_tiles:
		#print('get_cellv(tile)', get_cellv(tile))
		if get_cellv(tile) == 0:
			continue
...
func get_path_astart(start_tile, end_tile):
	#print("get_path_astart")
